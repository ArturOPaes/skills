...
2. **Action → consequence.** Perform every state-changing action, then check the UI reflects the new state **everywhere it appears** — counters, badges, quotas, lists, summaries. An action whose effect isn't visible where the user would look is a gap, even when the write succeeded.
