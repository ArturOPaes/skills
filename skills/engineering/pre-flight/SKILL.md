...
- **Mockup** — when the [mockup](../mockup/SKILL.md) is the project's canonical frontend source, **every FE definition** (screen, field, action, state) already appears in it — at the chosen fidelity, low- or hi-fi — and its screens match the blueprint's inventory. Anything the build would later put on the frontend that the mockup lacks is a **fail** — the mockup is extended first, never bypassed at implementation time.
