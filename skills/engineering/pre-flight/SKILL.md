...
You invoke this by typing `/pre-flight` — the agent won't reach for it on its own. It's the deliberate "is the whole thing designed enough to build?" checkpoint, run **once** — after the overview is drawn ([grill-design](../grill-design/SKILL.md) → [mockup](../mockup/SKILL.md) on the `mockup+hi-fi` path → [open-design](../open-design/SKILL.md) → [blueprint](../blueprint/SKILL.md)) and before implementation begins.

Reach for it to make sure nothing goes into code half-designed: that the docs, the diagrams, the design (the mockup only when the project's design flow uses one), and the open-design visual reference are all current, cover **every** user story, and have been reviewed. To arm the review it runs the pre-build **design eval** (the `verify-design` prototype) — a `US / ADR → screen` traceability matrix with per-item verdicts (present & faithful / shallow / absent / contradiction) — then walks the findings with you. It's what makes the design artifacts, open-design, blueprint, and grill-design effectively non-optional — the gate won't greenlight a product they haven't covered. Once it greenlights, the build phase (optional [to-waves](../to-waves/SKILL.md) → [to-tickets](../to-tickets/SKILL.md) → [implement](../implement/SKILL.md)) runs without re-gating — the invariant that no frontend exists outside the mockup keeps the overview true as the build proceeds.
...
- **Mockup** — on the `mockup+hi-fi` design flow, when the [mockup](../mockup/SKILL.md) is the project's canonical frontend source, **every FE definition** (screen, field, action, state) already appears in it at low-fi, and its screens match the blueprint's inventory. Anything the build would later put on the frontend that the mockup lacks is a **fail** — the mockup is extended first, never bypassed at implementation time. On the `hi-fi-only` flow there is no mockup, and this coverage check runs against the open-design reference instead.
- **Visual reference** — if the intent is a polished FE, an [open-design](../open-design/SKILL.md) hi-fi reference must exist here, covering the same screens: it's the visual target the shipped UI is required to match, and [manual-qa](../manual-qa/SKILL.md) checks that match after the build. The design eval verifies it **covers and faithfully expresses** every user story and frontend ADR — including that empty/loading/error states and the settled component-lib decisions are accounted for, since the HTML reference is look-only. No reference yet for a polished build is a **fail** (or a **recorded, deferred step** if visual polish is explicitly out of this milestone's scope), never an unstated gap.
