...
- **[blueprint](./blueprint/SKILL.md)** — Synthesize a spec's user stories, the domain model, and the UI into a `BLUEPRINT.md` of Mermaid diagrams (screen inventory, navigation map, per-screen field/action wireflows, user flow, traceability, states, sequence, data) for a macro visual view before implementation. No interview.
...
- **[pre-flight](./pre-flight/SKILL.md)** — Before building a wave, confirm its documentation, diagrams, low-fi mockup, and open-design visual reference are current and cover it, walk them with a human to review and adjust, then greenlight `/implement`. The pre-build twin of `/manual-qa`.
...
- **[prototype](./prototype/SKILL.md)** — Build a throwaway prototype to answer a design question: a runnable terminal app for state/logic, or, for a chosen platform (web/mobile/native), several toggleable UI variations with variant A recommended.
- **[mockup](./mockup/SKILL.md)** — Build a promotable low-fi mockup of a whole flow in the project's real UI stack, on a dedicated route with mock data, to align on coverage and structure before building — the opposite of a throwaway prototype. The hi-fi look is a separate reference; see `open-design`.
- **[open-design](./open-design/SKILL.md)** — Generate a flow's hi-fi visual design as a binding reference the shipped UI must match, from `DESIGN.md` and the mockup's screens, using Open Design (an AI design engine). Replaces the old hi-fi mockup; a standalone artifact you build toward, not code in the stack.
...
- **[critical-qa](./critical-qa/SKILL.md)** — Shared QA-critique discipline: generative probes (CRUD-completeness, action→consequence, state quality, native controls, shortest-path) and a grilling posture that surface what's missing and what's weak, not just whether the spec was met.
