...
- **[verify-spec](./verify-spec/SKILL.md)** — Post-implementation eval for a GitHub spec-kit feature: verifies the built thing satisfies every requirement in `spec.md` (FR / SC / acceptance scenarios) with real evidence, emitting a traceability matrix, per-requirement verdicts, and a done/not-done gate.
